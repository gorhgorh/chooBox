...
var soundArr = []
var sounds = Sounds()
sounds.init(files, pageInit)
// Create the object that contains functions that use web audio to
// make sound.
var play = sounds.playSound
var audio = sounds.ctx

function pageInit (sounds) {
  _.each(sounds, function (soundBuffer) {
    soundArr.push(soundBuffer)
  })
  // console.log('soundArr', soundArr)
  _.each(soundArr, function (sound) {
    addTrack(audio, sound)
  })
}
var colIt = 0
function addTrack (audio, sound) {
  var colors = ['blue', 'rebeccapurple', 'green', 'deeppink']
  data.tracks.push(createTrack(colors[colIt], play(audio, sound)))
  if (colIt < colors.length - 1) {
    colIt++
  } else colIt = 0
}

$('.playSound').click(function () {
  play(sounds.ctx, soundArr[0])
})

$('.playSound2').click(function () {
  play(sounds.ctx, soundArr[1])
})
...
var data = {
  ...
  tracks: [createTrack('gold', note(audio, 880)),
    ...
    createTrack('red', note(audio, 300)),
...
function kick (audio) {
  return function () {
    ...
    rampDown(audio, sineWave.frequency, 100, duration)
